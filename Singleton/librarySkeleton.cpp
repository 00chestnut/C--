...
public:
    ...
    void checkoutBook(Member &member, const string &title)
    {
    ...
    void returnBook(Member &member, const string &title)
    {
        Book *book = library.findBookByTitle(title);
        if (book)
        {
...
int main()
{
    ...
    // Create members
    Member alice("Alice", 1);
    Member bob("Bob", 2);

    // Check out books
    system.checkoutBook(alice, "1984");
    system.checkoutBook(bob, "To Kill a Mockingbird");

    // Try to check out already checked out books
    system.checkoutBook(bob, "1984");

    // Return books
    system.returnBook(alice, "1984");
    system.returnBook(bob, "To Kill a Mockingbird");
